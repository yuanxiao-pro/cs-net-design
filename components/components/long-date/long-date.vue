<template>
	<view>
		<view class="long-data">
			<view class="long-data-title long-data-fl">测量时间</view>
	...
	export default {
		...
		methods: {
			...
			bindDateChange(e) { //有效日期的滚动日期时间方法
				...
				let chooes_time = new Date(dateStr + ' ' + hourStr + ':' + minuteStr) //提示时间
				...
				this.selectRes = this.year + '-' + this.month + '-' + this.day + ' ' + this.hour + ':' + this.minute; //返回值
				//参数1是自定义事件
				this.$emit("select", 
					this.selectRes	
				);
