<template>
	<view class="uni-card uni-border" :class="{ 'uni-card--full': isFull === true || isFull === 'true', 'uni-card--shadow': isShadow === true || isShadow === 'true','uni-card-good':isGood === true || isGood === 'true','uni-card-law':isLaw === true || isLaw === 'true'}">
		<!-- 基础 -->
		...
		<view v-if="mode === 'title'" class="uni-card__title uni-border-bottom" @click.stop="onClick">
			<view class="uni-card__title-box">
				...
				<view class="uni-card__title-content">
					...
					<view class="timeBody" v-if="beginTime">
						<image src="../../static/tars/begin.png" class="time-icon"></image><text class="uni-card__title-content-extra uni-ellipsis">{{ beginTime }}</text>
					</view>
					<view class="timeBody" v-if="endTime">
						<image src="../../static/tars/end.png" class="time-icon"></image><text class="uni-card__title-content-extra uni-ellipsis">{{ endTime }}</text>
					</view>
				</view>
			</view>
			<view v-if="extra">
...
<style scoped>
	...
	.uni-card-law{
		box-shadow: 0px 0px 5px 1px #FF6699;
	}
	
	.uni-card-good{
		box-shadow: 0px 0px 5px 1px #83DC42;
	}
